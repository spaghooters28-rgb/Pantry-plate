...
Pantry & Plate is a full-stack meal-planning application with a React SPA frontend and an Express API backed by PostgreSQL via Drizzle ORM. Users can register/login, manage pantry and grocery data, generate weekly meal plans, and use AI-powered recipe ingestion and chat features. Production scope is `artifacts/api-server`, `artifacts/meal-planner`, and shared libraries under `lib/`; `artifacts/mockup-sandbox` is development-only and should be ignored unless production reachability is demonstrated.
...
- **Database integrity** — meals, ingredients, sides, and shared catalog data. Unauthorized writes can poison recommendations and corrupt other users' application state.
...
- **Highest-risk API areas**: `artifacts/api-server/src/routes/openai/index.ts`, `artifacts/api-server/src/routes/analyze-recipe.ts`, `artifacts/api-server/src/routes/ai-meals.ts`, and authenticated CRUD routes under `grocery.ts`, `pantry.ts`, `history.ts`, `schedule.ts`, and `weekly.ts`.
- **Public surfaces**: health, auth, meal listing/detail, AI meal generation, recipe analysis/save, and OpenAI conversation routes unless protected in code.
